...
def build_qwen3_decode_program():
    ...
    class Qwen3Decode_OPT:
        ...
        def qwen3_decode(
            ...
            for ob in pl.range(0, HIDDEN // SCOPE3_OUT_CHUNK):
                ...
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="out_proj"):
                    ...
                    for kb in pl.pipeline(2, HIDDEN // OUT_PROJ_K_CHUNK, stage=2):
                        ...
                        w_chunk = wo[k0 : k0 + OUT_PROJ_K_CHUNK, o0 : o0 + SCOPE3_OUT_CHUNK]
                        o_acc = pl.matmul_acc(o_acc, a_chunk, w_chunk)
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="out_residual"):
                    hidden_chunk = hidden_states[:, o0 : o0 + SCOPE3_OUT_CHUNK]
            ...
            for o0 in pl.range(0, INTERMEDIATE, MLP_OUT_CHUNK):
            ...
            for db in pl.range(0, HIDDEN // DOWN_N_CHUNK):
                d0 = db * DOWN_N_CHUNK
                ...
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="down_residual"):
