...
def build_decode_projection_program(
    ...
    class DecodeProjectionTileProgram:
        ...
        @pl.function(type=pl.FunctionType.InCore)
        def kv_proj_fused_reduce(
            self,
            normed_tile: pl.Tensor[[BATCH_TILE, hidden], pl.BF16],
            weight_k: pl.Tensor[[hidden, kv_hidden], pl.BF16],
            weight_v: pl.Tensor[[hidden, kv_hidden], pl.BF16],
            out_row: pl.Scalar[pl.INDEX],
            out_col: pl.Scalar[pl.INDEX],
            output_k: pl.Out[pl.Tensor[[batch, kv_hidden], pl.FP32]],
            output_v: pl.Out[pl.Tensor[[batch, kv_hidden], pl.FP32]],
        ) -> tuple[
            pl.Tensor[[batch, kv_hidden], pl.FP32],
            pl.Tensor[[batch, kv_hidden], pl.FP32],
        ]:
            tile_a_l1 = pl.load(normed_tile, [0, 0], [BATCH_TILE, K_CHUNK], target_memory=pl.MemorySpace.Mat)
            tile_bk_l1 = pl.load(weight_k, [0, out_col], [K_CHUNK, KV_OUT_CHUNK], target_memory=pl.MemorySpace.Mat)
            tile_bv_l1 = pl.load(weight_v, [0, out_col], [K_CHUNK, KV_OUT_CHUNK], target_memory=pl.MemorySpace.Mat)
            tile_a_l0a = pl.move(tile_a_l1, target_memory=pl.MemorySpace.Left)
            tile_bk_l0b = pl.move(tile_bk_l1, target_memory=pl.MemorySpace.Right)
            acc_k = pl.matmul(tile_a_l0a, tile_bk_l0b)
            tile_bv_l0b = pl.move(tile_bv_l1, target_memory=pl.MemorySpace.Right)
            acc_v = pl.matmul(tile_a_l0a, tile_bv_l0b)

            for kb in pl.range(1, hidden_blocks):
                k0 = kb * K_CHUNK
                tile_a_i_l1 = pl.load(normed_tile, [0, k0], [BATCH_TILE, K_CHUNK], target_memory=pl.MemorySpace.Mat)
                tile_bk_i_l1 = pl.load(weight_k, [k0, out_col], [K_CHUNK, KV_OUT_CHUNK], target_memory=pl.MemorySpace.Mat)
                tile_bv_i_l1 = pl.load(weight_v, [k0, out_col], [K_CHUNK, KV_OUT_CHUNK], target_memory=pl.MemorySpace.Mat)
                tile_a_i_l0a = pl.move(tile_a_i_l1, target_memory=pl.MemorySpace.Left)
                tile_bk_i_l0b = pl.move(tile_bk_i_l1, target_memory=pl.MemorySpace.Right)
                acc_k = pl.matmul_acc(acc_k, tile_a_i_l0a, tile_bk_i_l0b)
                tile_bv_i_l0b = pl.move(tile_bv_i_l1, target_memory=pl.MemorySpace.Right)
                acc_v = pl.matmul_acc(acc_v, tile_a_i_l0a, tile_bv_i_l0b)

            out_k = pl.store(acc_k, [out_row, out_col], output_k)
            out_v = pl.store(acc_v, [out_row, out_col], output_v)
            return out_k, out_v
        ...
        def decode_projection(
            ...
            for b0 in pl.range(0, batch, BATCH_TILE):
                ...
                for ob in pl.range(kv_out_blocks):
                    kv0 = ob * KV_OUT_CHUNK
                    k_proj, v_proj = self.kv_proj_fused_reduce(normed_tile, wk, wv, b0, kv0, k_proj, v_proj)
...
if __name__ == "__main__":
    ...
    result = run(
        ...
        config=RunConfig(
            ...
            compile=dict(
                dump_passes=True,
                output_dir=f"build_output/DecodeProjectionTileProgram_opt_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            ),
            runtime=dict(
